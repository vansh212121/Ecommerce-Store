...
class SizeRepository(BaseRepository[Size]):
    """Repository for all database operations related to the Size model."""
    ...
    async def get(self, db: AsyncSession, *, obj_id: uuid.UUID) -> Optional[Size]:
        """get size by its ID"""
    ...
    async def get_by_name(self, db: AsyncSession, *, name: str) -> Optional[Size]:
        """Get size by name"""
    ...
    async def get_all(
        ...
        *,
        ...
        """Get multiple sizes with filtering and pagination."""
    ...
    async def create(self, db: AsyncSession, *, db_obj: Size) -> Size:
        """Create a new size. Expects a pre-constructed Size model object."""

        db.add(db_obj)
        await db.commit()
        await db.refresh(db_obj)
        self._logger.info(f"Size created: {db_obj.id}")
        return db_obj
    ...
    async def update(
        self, db: AsyncSession, *, size: Size, fields_to_update: Dict[str, Any]
    ) -> Size:
        """Updates specific fields of a size object."""
        ...
        self._logger.info(
            f"Size fields updated for {size.id}: {list(fields_to_update.keys())}"
        )
        return size
    ...
    async def delete(self, db: AsyncSession, *, obj_id: uuid.UUID) -> None:
        """Permanently delete a size by ID."""
        statement = delete(self.model).where(self.model.id == obj_id)
        await db.execute(statement)
        await db.commit()
        self._logger.info(f"Size hard deleted: {obj_id}")
        return
